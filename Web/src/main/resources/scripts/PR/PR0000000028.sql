 ...
 /*        SELECT  SOLICITUDES.FIFOLIOSOLICITUD,
    ...
    OPEN curCursorSalida FOR
        ...
        WHERE   SD.FIESTATUSSOL IN (SELECT * FROM TABLE(    CASE paNoTipoSolicitud
                                                                ...
                                                                WHEN csgNoActNotaCargoEntrada THEN sys.odcinumberlist(csgSolicitadoCD,csgAtendidoCD,csgCaminoTienda,csgRecibidoTienda,csgEntregado)
                                                            END)) AND
 ...
 *************************************************************/             
 ...
 BEGIN	
    ...
    FOR i IN paSolicitudesDetalle.FIRST..paSolicitudesDetalle.LAST LOOP
        ...
        vlNoEstatusSolicitud := PASRVUNIFORMESCOMERCIO.FNESTATUSCDRUTADESCXSOLICITUD(   paSolicitudesDetalle(i).FIESTATUSCD,
        ...
        IF paSolicitudesDetalle(i).FIESTATUSSOL <> vlNoEstatusSolicitud THEN 
            ...
            vlNoEstatusAnterior := paSolicitudesDetalle(i).FIESTATUSCD;
            ...
            PAWEBUNIFORMESCOMERCIO.SPGUARDABITACORA(   
                ...
                vlNoEstatusAnterior,
                vlNoEstatusNuevo,
                vlCadenaComentariosBitacora || 
                ', FIESTATUSSOL(' ||
                TO_CHAR(vlNoEstatusSolicitud) ||
                '), FIESTATUSCD(' ||
                TO_CHAR(paSolicitudesDetalle(i).FIESTATUSCD) || 
                ')',
                csgCero);  
            END IF;                                           
        END LOOP;
 ...
 *************************************************************************************************************/
 ...
 BEGIN
    IF paNoEstatusCD = 0 AND UPPER(trim(paDescripcionBitacora)) = csgCadenaGrabadoEnTienda THEN -- (SIN ESTATUS WSABASTO) SOLICITUD EN TIENDA
        vlNoEstatusSolicitud := 2; --SOLICITADO EN TIENDA
    ELSIF paNoEstatusCD = 0 AND UPPER(trim(paDescripcionBitacora)) = csgCadenaGrabadoEnCD THEN --PEDIDO CREADO (CD)                      
        vlNoEstatusSolicitud := 3; --SOLICITADO EN CD
    ELSIF paNoEstatusCD IN (20,200) AND UPPER(trim(paDescripcionBitacora)) != csgCadenaGrabadoEnTienda THEN --PEDIDO CREADO (CD)                      
        vlNoEstatusSolicitud := 3; --SOLICITADO EN CD
    ELSIF paNoEstatusCD IN (400,450,492,496,500,600,650) AND paDescripcionCD NOT LIKE csgCadenaFailed THEN --ASIGNADO EN ALMACEN (CD)          
        vlNoEstatusSolicitud := 4; --ATENDIDO EN CD
    ELSIF paNoEstatusCD IN (400,450,492,496,500,600,650) AND paDescripcionCD LIKE csgCadenaFailed THEN --ASIGNADO EN ALMACEN (CD)                
        vlNoEstatusSolicitud := 4; --ATENDIDO EN CD            
    ELSIF paNoEstatusCD = 490 THEN --ATENDIENDO SOLICITUD                
        vlNoEstatusSolicitud := 4; --ATENDIDO EN CD   
    ELSIF paNoEstatusCD = 850 AND paNoRuta = 0 THEN --REMISIONADO SIN RUTA (CD)              
        vlNoEstatusSolicitud := 4; --ATENDIDO EN CD           
    ELSIF paNoEstatusCD = 850 AND paNoRuta > 0 THEN --ENVIADO A TIENDA              
        vlNoEstatusSolicitud := 5; --EN CAMINO A TIENDA
    ELSIF paNoEstatusCD = 940 OR UPPER(trim(paDescripcionCD)) = csgCadenaCancelado THEN --CANCELADO
